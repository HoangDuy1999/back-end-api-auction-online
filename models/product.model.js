...
module.exports = {
  findAll(condition_end_day = false) {
    if (condition_end_day) {
      return db.select('p.*', 'a.count_auction', 't.name as type_name', 't.alias as type_alias',
        ...
        .leftJoin('product as acc1', 'acc1.product_id', 'p.seller_id')
        .leftJoin('product as acc2', 'acc2.product_id', 'a.bidder_id')
  ...
  getInfoAuctioneer(product_id) {
    return db.select('ad.*', 'ra.product_id', 'acc.full_name', 'ra.reason')
      .from('auction as a')
      ...
      .leftJoin('product as acc', 'ra.product_id', 'acc.product_id')
